...
export default function CommitList({ commits, isLoading, error, timeRange }: CommitListProps) {
  const { mutate: generateEmail, data: emailData, isPending: isGenerating, error: emailError } = useGeminiEmail();

  const handleGenerateEmail = () => {
    if (commits.length > 0 && timeRange) {
      generateEmail({
        commits,
        timeRange
      });
    ...
    <div className="w-full max-w-lg space-y-6">
      ...
      {(isGenerating || emailData || emailError) && (
        <EmailGenerator
          ...
          error={emailError instanceof Error ? emailError.message : emailData?.error}
        />
      )}
    </div>
  );
